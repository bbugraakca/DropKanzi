...
export type PriceBreakdown = {
  ...
  /** @deprecated use profit */
  profitAmount: number;
  /** @deprecated use marginFixed */
  fixProfit: number;
  /** @deprecated */
  extraFee: number;
...
function applyRoundPrices(price: number, roundPrices: any): number {
  ...
  if (mode === "NONE") return roundMoney(price);
  ...
  if (mode === "CUSTOM_STEP") {
    const step = Number(roundPrices?.customStep || 0);
    if (!step || step <= 0) return roundMoney(price);
    return roundMoney(Math.round(price / step) * step);
  }
  return roundMoney(Math.floor(price) + 0.99);
}

function readFeeSettings(settings: Settings) {
  const additionalFee = settings?.additionalFee || {};
  const repricing = settings?.repricingSettings || {};
  ...
  let ebayFeePercent = Number(
    additionalFee.ebayFeePercent ?? additionalFee.percentageFee ?? additionalFee.extraFeePercent ?? 13
  );
  ...
  const easyncAoFee = Number(repricing.easyncAoFee ?? additionalFee.easyncAoFee ?? 0);
  const minProfit = Number(repricing.minProfit ?? 0);
  const addonsMargin = Number(repricing.addonsMargin ?? 0);
...
export function calcSuggestedEbayPrice(input: {
  ...
  const ranges: RangeRow[] = settings.rangeRepricing?.ranges || [];
  ...
  // Profit = source * margin% * 0.01 + margin fixed + addons margin
  let profit =
    sourcePrice * marginPercent * 0.01 + marginFixed + fees.addonsMargin;
  if (profit < fees.minProfit) {
    profit = fees.minProfit;
  }

  // Price = (source + profit + easync AO) / (1 - (ebay% + paypal%) * 0.01) + fixed PayPal + fixed fee
  ...
  if (divisor <= 0.01) {
    priceBeforeVat = numerator + fees.fixedPaypalFee + fees.fixedFee;
  } else {
    priceBeforeVat = numerator / divisor + fees.fixedPaypalFee + fees.fixedFee;
  }

  const vatCfg = settings.vatDetails || settings.vat || {};
  const vatEnabled = !!(vatCfg.vatEnabled ?? vatCfg.enabled);
  const vatPercent = vatEnabled
    ? Number(vatCfg.vatRatePercent ?? vatCfg.vatPercent ?? 0)
    : 0;
  const vatAmount = vatEnabled ? priceBeforeVat * (vatPercent * 0.01) : 0;

  const priceBeforeRounding = priceBeforeVat + vatAmount;
  const priceAfterRounding = applyRoundPrices(priceBeforeRounding, settings.roundPrices);
  ...
  const breakdown: PriceBreakdown = {
    ...
    priceAfterRounding,
    profitAmount: roundMoney(profit),
    fixProfit: roundMoney(marginFixed),
    extraFee: roundMoney(fees.fixedFee + fees.fixedPaypalFee),
    vat: roundMoney(vatAmount),
    base: roundMoney(sourcePrice),
  };
